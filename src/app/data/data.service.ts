...
export class DataService {
  ...
  isLoggedIn(){
  return this.http.get('http://localhost:3001/isLoggedIn');
  }
    ...
    return this.http.get<NoteResponse[]>('http://localhost:3001/notes/' + id).pipe(
      tap(notesResponse => {
        let notes = notesResponse.map(noteResponse => {
          let note = new Note(noteResponse.title, noteResponse.content, noteResponse.category_id, noteResponse.updated_at, noteResponse.id);
          return note;
        });
